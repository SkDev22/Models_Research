...
class LocationFeatures:
    def __init__(self):
        self.university = ""              # Name of the nearest university
        self.distance_to_uni = 0.0        # in km
        self.distance_to_transport = 0.0   # distance to nearest bus stand in km
    ...
    def calculate_location_score(self):
        ...
        if self.distance_to_transport < 0.5:
            score += 3.5  # Increased weighting to compensate for removed food score
...
def recommend_price_range(distance, amenity_score):
    """
    Recommend a price range based on distance to university and amenities.
    amenity_score: 0-10 scale (higher is better)
    """
    # Distance-based base ranges (updated)
    if distance <= 0.5:
        base_min, base_max = 45000, 60000
    elif distance <= 3:
        base_min, base_max = 20000, 30000
    elif distance <= 10:
        base_min, base_max = 10000, 20000
    else:
        base_min, base_max = 8000, 15000
    # Adjust by amenities: shift up to +20% for great amenities, -15% for poor
    # Amenity score: 0-10, neutral at 5
    if amenity_score >= 8:
        adj_min = int(base_min * 1.15)
        adj_max = int(base_max * 1.2)
    elif amenity_score <= 3:
        adj_min = int(base_min * 0.85)
        adj_max = int(base_max * 0.9)
    else:
        # Linear scale between 0.85 and 1.15 for min, 0.9 and 1.2 for max
        factor_min = 0.85 + 0.06 * (amenity_score-3) if amenity_score < 8 else 1.15
        factor_max = 0.9 + 0.06 * (amenity_score-3) if amenity_score < 8 else 1.2
        adj_min = int(base_min * factor_min)
        adj_max = int(base_max * factor_max)
